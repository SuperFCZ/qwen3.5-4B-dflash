...
INSTALL_DIR=/home/w00949577/z50058744/custom_ops_runtime/smoke_matmul
...
mkdir -p "$build_root"
rm -rf -- "$op_project"
msopgen gen -i "$here/SmokeMatmul.json" -c "ai_core-$soc_version" -lan cpp -out "$op_project"
cp "$here/op_host/smoke_matmul.cpp" "$here/op_host/smoke_matmul_tiling.h" "$op_project/op_host/"
cp "$here/op_kernel/smoke_matmul.cpp" "$op_project/op_kernel/"
(
    cd "$op_project"
    bash build.sh
)
...
if [[ ! -f "$vendor_api/include/aclnn_smoke_matmul.h" ]]; then
    echo "Installed ACLNN header missing: $vendor_api/include/aclnn_smoke_matmul.h" >&2
    exit 1
fi
...
"$python_bin" "$here/test/reference.py" prepare "$build_root/data"
"$build_root/test/smoke_matmul_test" "${DEVICE_ID:-0}" "$build_root/data"
"$python_bin" "$here/test/reference.py" check "$build_root/data"
