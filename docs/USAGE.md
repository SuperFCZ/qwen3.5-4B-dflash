...
准备 Ascend310P 的 CANN、PyTorch/torch_npu、TorchAir、自定义算子包和 Target/Draft 权重。
receiver 提供 `models/export_model_wrapper_qwen3_5.py`；
Chunk 算子支持 INT16 effective_length，MTP 路线另需 GDR MTP 算子。
...
仅填写所选 Draft 路径；新终端或修改配置后重新 source。
...
- `nz` 从 W8 checkpoint 自动离线打包并校验，直接写入 AIR；只支持 `weight_quant`，无需额外转换脚本。
...
离线 NZ W8 自动编译 C16/C64 静态档位；`--resume` 校验并复用已完成产物。
加 `--draft-quantizations w8a16` 可只编译 W8。日志在 `$AI_RUN_DIR/log/dflash-atc/`。
...
每题每输出上限的 ordinary 只测一组，各 Draft/Verify 共用基线。
...
复用 ordinary 须匹配模型、runner、设备、输入、thinking、输出上限和重复次数。
严格 token 对照去掉 `--allow-output-differences`；性能口径及结果见 [测试结果](RESULTS.md)。
